...
class CPD_SecondOrderSDPScorer(nn.Module):
    def __init__(self, conf, fields, input_dim):
        super(CPD_SecondOrderSDPScorer, self).__init__()
        self.conf = conf
        ...
        self.sib_scorer = DecomposedPentaAffineScorer(n_in=input_dim, n_out=conf.n_mlp_arc,
                                          dropout=conf.mlp_dropout, rank=conf.rank, scaling=self.conf.scaling)

        self.cop_scorer = DecomposedPentaAffineScorer(n_in=input_dim, n_out=conf.n_mlp_arc,
                                          dropout=conf.mlp_dropout, rank=conf.rank, scaling=self.conf.scaling)

        self.grd_scorer = DecomposedPentaAffineScorer(n_in=input_dim, n_out=conf.n_mlp_arc,
                                          dropout=conf.mlp_dropout, rank=conf.rank, scaling=self.conf.scaling)

        self.grd_scorer2 = DecomposedPentaAffineScorer(n_in=input_dim, n_out=conf.n_mlp_arc,
                                          dropout=conf.mlp_dropout, rank=conf.rank, scaling = self.conf.scaling)
