...
class SeparateLSTMencoder(nn.Module):
    def __init__(self, conf, input_dim, **kwargs):
        super(SeparateLSTMencoder, self).__init__()
        self.conf = conf
        self.lstm_dep = LSTMencoder(conf, input_dim, **kwargs)
        self.lstm_const = LSTMencoder(conf, input_dim,  **kwargs)

    def forward(self, ctx):
        self.lstm_dep(ctx)
        ctx['x_dep'] = ctx['encoded_emb'].clone()
        self.lstm_const(ctx)
        ctx['x_const'] = ctx['encoded_emb'].clone()
        ctx['encoded_emb'] = None
