...
class DataModuleBase(pl.LightningDataModule):
    ...
    def _build_dataset_and_fields(self):
        ...
        if os.path.exists(self.conf.cache) and self.conf.use_cache:
            ...
            datasets = self._post_process_dataset(datasets)
            self._dump(cache_path=self.conf.cache, to_cache={'datasets': datasets,
                         'fields': fields})
        return datasets, fields


    # sometimes we need get indexed label. so we have to wait until the dataset is built.
    def _post_process_dataset(self, datasets):
        for name, dataset in datasets.items():
            datasets[name] =  self._post_process(dataset)
